...
airTavg2024<-daily_data %>% 
  ...
  filter(Year>2023) %>% 
  group_by(doy) %>% 
  summarize(air_max_2024 = max(air_temp_max_5m, na.rm =TRUE),
            air_min_2024 = min(air_temp_min_5m, na.rm =TRUE),
            air_max_avg_2024 = mean(air_temp_max_5m, na.rm = TRUE),
            air_min_avg_2024 = mean(air_temp_min_5m, na.rm = TRUE))
...
p<-ggplot() +
  ...
  geom_ribbon(data = airTavg2024,aes(x = doy,ymin = air_min_avg_2024,ymax = air_max_avg_2024), fill = "red4", alpha=0.6, show.legend = FALSE) +
...
#  geom_line(data=lake_temp, aes(x=doy, y=lake_avg), colour="aquamarine4", size=1.5, show.legend = FALSE)+
  ...
  theme_bw()+
  ggtitle("Through the year at Toolik", subtitle = "Data from the TFS Met Station and Naturalist Journal (2024)")+
...
p3<-ggplot()+
  ...
  annotate("segment", x=0, xend=38, y=81, yend=55, colour="#5b5b5b", size=0.35)+
  annotate("segment", x=17, xend=29, y=1, yend=14, colour="#5b5b5b", size=0.35)+
  ...
  annotate("text", x = 8, y = 92, label = "Temperature Range", size=2.5, colour="#5b5b5b")+
  ...
  annotate("text", x = 1.5, y = 89, label = "2024", size=2.5, hjust=0, colour="#5b5b5b")+
  annotate("text", x = 1.5, y = 86, label = "Average", size=2.5, hjust=0, colour="#5b5b5b")+
  annotate("text", x = 1.5, y = 83, label = "Extreams", size=2.5, hjust=0, colour="#5b5b5b")+
#  annotate("text", x = 1.5, y = 85, label = "Average Lake", size=2.5, hjust=0, colour="#5b5b5b")+
  ...
  annotate("point",x = 0, y = 89, shape = 15, colour = "red4", alpha = 0.6, size = 3)+
  annotate("point",x = 0, y = 86, shape = 15, colour = "red", alpha = 0.5, size = 3)+
  annotate("point",x = 0, y = 83, shape = 15, colour = "red", alpha = 0.1, size = 3)+
...
p2 + inset_element(p3, left = 0, bottom = 0, right = 1, top = 1)+
  labs(caption = "Amanda B. Young, 2025", fontface = "italic")+
